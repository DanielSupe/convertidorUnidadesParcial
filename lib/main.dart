...
class _Convertidor extends State<Convertidor> {
  ...
  final List<Map<String, dynamic>> mapeado = [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(mapeado[actualPage]['title'],style: TextStyle(color: Colors.white)),
        backgroundColor:  mapeado[actualPage]['Color'],
      ),
      drawer: Drawer(
        child: ListView(
          children: [
            Container(
              height: 70,
              child: DrawerHeader(
                decoration: BoxDecoration(color: mapeado[actualPage]['Color']),
                child: Text(
                  'Categorías',
      ...
      body: mapeado[actualPage]['component'],
    );
  }
}
